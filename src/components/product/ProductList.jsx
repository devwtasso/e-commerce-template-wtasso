...
const ProductList = (props) => {
  ...
  if (filteredProducts.length === 0 && !isLoading) {
    return (
      <MessageDisplay message={requestStatus?.message || 'Produtos não encontrados.'} />
    );
  } if (filteredProducts.length === 0 && requestStatus) {
    return (
      <MessageDisplay
        message={requestStatus?.message || 'Algo deu errado :('}
      ...
      {products.items.length < products.total && (
        <div className="d-flex-center padding-l">
          <button
            ...
            type="button"
          >
            {isFetching ? 'Carregando Itens...' : 'Mostrar mais'}
          </button>
        </div>
      )}
    </Boundary>
