    ...
    case UPDATE_PROFILE: {
      try {
        ...
        if (e.code === 'auth/wrong-password') {
          yield call(displayActionMessage, 'Senha incorreta, falha ao atualizar perfil :(', 'error');
        } else {
          yield call(displayActionMessage, `:( Falha ao atualizar perfil. ${e.message ? e.message : ''}`, 'error');
        }
      }
      break;
    }
